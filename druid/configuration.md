<!-- TITLE: Configuration -->
<!-- SUBTITLE: A quick summary of Configuration -->
...
## Exhibitor
[Exhibitor](https://github.com/soabase/exhibitor/wiki) is a supervisor system for ZooKeeper. Exhibitor can dynamically scale-up/down the cluster of ZooKeeper servers. Druid can update self-owned list of ZooKeeper servers through Exhibitor without restarting. That is, it allows Druid to keep the connections of Exhibitor-supervised ZooKeeper servers.
...
_Note that `druid.zk.service.host` is used as a backup in case an Exhibitor instance can't be contacted and therefore should still be set._
...
_Note that some sensitive information may be logged if these settings are enabled._
...
Note that, you can enable sending all the HTTP requests to log by setting "io.druid.jetty.RequestLog" to DEBUG level. See [Logging](http://druid.io/docs/0.12.0/configuration/logging.html)
...
For configuring the above request logging types, see [Configuration](http://druid.io/docs/0.12.0/configuration/index.html) section 'Request Logging'.

## Emitting Metrics
The Druid servers emit [various metrics](http://druid.io/docs/0.12.0/operations/metrics.html) and alerts via something we call an Emitter. There are three emitter implementations included with the code, a "noop" emitter, one that just logs to log4j ("logging", which is used by default if no emitter is specified) and one that does POSTs of JSON events to a server ("http"). The properties for using the logging emitter are described below.
...
For configuring the above Emitter modules, see [Configuration](http://druid.io/docs/0.12.0/configuration/index.html) section 'Emitter Metrics'.

## Metadata Storage
These properties specify the jdbc connection and other configuration around the metadata storage. The only processes that connect to the metadata storage with these properties are the [Coordinator](http://druid.io/docs/0.12.0/design/coordinator.html), [Indexing service](http://druid.io/docs/0.12.0/design/indexing-service.html) and [Realtime Nodes](http://druid.io/docs/0.12.0/design/realtime.html).
...
## Deep Storage
The configurations concern how to push and pull [Segments](http://druid.io/docs/0.12.0/design/segments.html) from deep storage.
...
For other Deep Storage Interfaces, see [Configuration](http://druid.io/docs/0.12.0/configuration/index.html) section 'Deep Storage'.
...
This config is used to find the [Indexing Service](http://druid.io/docs/0.12.0/design/indexing-service.html) using Curator service discovery. Only required if you are actually running an indexing service.
...
This config is used to find the [Coordinator](http://druid.io/docs/0.12.0/design/coordinator.html) using Curator service discovery. This config is used by the realtime indexing nodes to get information about the segments loaded in the cluster.
