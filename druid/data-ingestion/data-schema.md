<!-- TITLE: Data Schema & Ingestion Specifications -->
<!-- SUBTITLE: A quick summary of Data Schema & Ingestion Specifications -->
...
If `type` is not included, the parser defaults to `string`. For additional data formats: [extensions list](http://druid.io/docs/0.12.0/development/extensions.html)
...
_If format is not included, the parseSpec defaults to tsv._
...
| flattenSpec    | JSON Object | Specifies flattening configuration for nested JSON data. See [Flattening JSON](http://druid.io/docs/0.12.0/ingestion/flatten-json.html) for more info. | no       |
...
3. The timestamp column and columns/fieldNames required by metrics are excluded by default.
