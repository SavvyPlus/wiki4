<!-- TITLE: Data Formats -->
<!-- SUBTITLE: A quick summary of Data Formats -->

# Data Formats for Ingestion

Druid can ingest denormalized data in **JSON, CSV, or a delimited form such as TSV**, or any custom format. 

For additional data formats, see the [extensions list](http://druid.io/docs/0.12.0/development/extensions.html).
...
Also, you can skip some header rows by setting `skipHeaderRows` in your `parseSpec`. If both `skipHeaderRows`and `hasHeaderRow`options are set, `skipHeaderRow` is first applied. For example, if you set `skipHeaderRows` to 2 and `hasHeaderRow` to true, Druid will skip the first two lines and then extract column information from the third line.
...
The columns field must be included to ensure that the order of the fields matches the columns of your input data in the same order.
